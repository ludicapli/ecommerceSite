...
import React, { createContext, useContext, useMemo, useEffect, useRef, useState, useCallback, startTransition, createRef, useLayoutEffect, forwardRef } from 'react';
import { defer, tap, switchMap, of, EMPTY, concatMap, Subject } from 'rxjs';
import { Schema } from '@sanity/schema';
import debug$l from 'debug';
import { randomKey } from '@sanity/util/content';
import { ReactEditor, withReact, Slate, useSlate, useSelected, useSlateStatic, Editable } from '@sanity/slate-react';
import { Element as Element$1, Text, Editor, Transforms, Range, Node, Operation, Path, createEditor } from 'slate';
  ...
  return function withPatches(editor) {
    ...
    if (patches$) {
      editor.subscriptions.push(() => {
        ...
        const sub = patches$.pipe(tap(_ref6 => {
          let {
            patches
          } = _ref6;
        ...
        }), bufferUntil(() => !isPending.current), concatMap(incoming => {
          return incoming;
        })).subscribe(_ref7 => {
          let {
            patches,
    ...
    currentBlock.children.forEach((currentBlockChild, currentBlockChildIndex) => {
      ...
      if (isChildChanged) {
        if (currentBlockChild._key === (oldBlockChild == null ? void 0 : oldBlockChild._key)) {
          debug$5("Updating changed child", currentBlockChild);
...
function Synchronizer(props) {
  ...
  const onFlushPendingPatchesDebounced = useMemo(() => debounce(onFlushPendingPatches, FLUSH_PATCHES_DEBOUNCE_MS, {
    leading: false,
    trailing: true
